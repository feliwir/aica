...
void aica::Network::Train(const xt::xarray<float> &inputs, const xt::xarray<float> &targets)
{
    ...
    m_who += m_learningRate * xt::linalg::dot((outputErrors * finalOutputs * (1.0f - finalOutputs)), tmp_t);

    tmp_t = xt::transpose(inputs);
    m_wih += m_learningRate * xt::linalg::dot((hiddenErrors * hiddenOutputs * (1.0f - hiddenOutputs)), tmp_t);
}
...
// void aica::Network::Train(xt::xarray<float> inputs, xt::xarray<float>
// targets)
//{
